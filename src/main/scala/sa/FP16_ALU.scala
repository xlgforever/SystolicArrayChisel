...
///////////////////////////////////////////////////////////// stage 3 /////////////////////////////////////////////////////////////////
  // lzd
  val lzd = Module(new LZD_Wrapper(cfg.sigBits*2))
  lzd.io.data := fracMul_s2
  val lzdCount = RegNext(lzd.io.zcnt)
  val lzdZero  = RegNext(lzd.io.allZero)
  // 与lzd的结果对齐时钟
  val fracMul_s3 = RegNext(fracMul_s2)
  val fix_exp = RegNext(fix_exp_wire) // 范围为 -15 ~ +60
...
///////////////////////////////////////////////////////////////////////stage 4//////////////////////////////////////////////
  ...
  val exp_shift = Mux(exp_all_wire_msb, exp_shift0, lzdCount) // 当exp_all_wire为负数时(此时不考虑fix_exp+1本来就为负数），表示指数部分不够lzd的大小，最多只能右移exp_shift0位，否则右移lzdCount位
  dontTouch(exp_shift)
...
////////////////////////////////////////////////////////////////////stage 5//////////////////////////////////////////////
  ...
  final_frac := Mux(exp_all.orR, frac_all << 1.U, frac_all)
  // 2. 当包含整数部分的frac_all不为全0时，指数部分不变；档次包含整数部分的frac_all全为0时，指数部分也要为0，表示结果为0
  val final_exp = Mux(frac_all.orR, exp_all, 0.U)
  dontTouch(final_exp)
...
////////////////////////////////////////////////////////////////////stage 6//////////////////////////////////////////////
  ...
  val fp_c = Mux(isSpecial_s5, specialResult_s5, Cat(sign_s5, final_exp_wire, final_frac_wire))
